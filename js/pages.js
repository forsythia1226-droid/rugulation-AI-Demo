...
function renderMyReq(){
  ...
  <section class="card pad">
   ...
   <form class="form rq" id="rqf">
    <label class="w2">규정<select id="rq-reg" required><option value="">규정을 선택하세요</option>
     ${regs.map(k=>`<option value="${k}" ${k===reqSel?"selected":""}>${esc(D[k].no)} ${esc(t(short(D[k].name)))}${D[k].parent?" ("+t("지침")+")":""}</option>`).join("")}</select></label>
    <label>담당팀 · 담당자<input id="rq-own" readonly value="${o?`${esc(t(D[reqSel].owner))} · ${esc(o.main?t(o.main):t("담당자 미지정"))}`:""}" placeholder="규정을 선택하면 자동으로 지정됩니다"></label>
 ...
 $("#rq-send").onclick=()=>{
  ...
  if(!k||!q){alert(t("규정과 질문을 모두 입력해 주세요."));return;}
  ...
  escStore.add(e);
  reqSel="";renderMyReq();
  $("#rq-done").innerHTML=`<p class="rq-sent">${IC.check}<span><b>${esc(t(e.assignee?nameOnly(e.assignee):d.owner))}</b> ${t("님에게 Teams 알림을 보냈습니다. 답변이 오면 아래 내역에 표시됩니다.")}</span></p>`;
...
function teamsWindow(e){
 ...
 m.innerHTML=`<div class="tw" role="dialog" aria-label="${t("Teams 채팅 미리보기")}">
  ...
  <p class="tw-note">${t("시연 화면입니다. 운영 시에는 서버가 담당자 Teams로 이 메시지를 자동 전송하고, 담당자가 남긴 답변이 문의자의 규정 문의 내역에 표시됩니다.")}</p>
 </div>`;
...
@media print{body{margin:0}h2,h3{break-after:avoid}table,.art{break-inside:avoid}}`;
 const body=(d.chapters||[]).map(ch=>`<h2>${esc(t(ch.t))}</h2>`+ch.arts.map(a=>`<div class="art"><h3>${esc(t(a.n))}(${esc(t(a.h))})</h3>
  ${a.body.map(b=>b==="TABLE"?tableHTML(a.table):`<p>${esc(t(b))}</p>`).join("")}${a.xref?`<p class="xref">${t("연계")}: ${esc(t(a.xref))}</p>`:""}</div>`).join("")).join("");
 return `<!DOCTYPE html><html lang="${lang()}"><head><meta charset="utf-8"><title>${esc(d.no)} ${esc(t(d.name))}</title><style>${css}</style></head><body>
<h1>${esc(t(short(d.name)))}</h1><p class="sub">${esc(d.no)} · ${t("시행")} ${esc(d.effective)} · ${t("주관")} ${esc(t(d.owner))}</p>
<p class="warn">${t("사내 한정 · 시연용 가상 조문입니다. 출력·저장본은 최신 개정이 반영되지 않을 수 있으니 시스템에서 최신본을 확인하십시오.")}</p>
${body||"<p>등록된 조문이 없습니다.</p>"}
<p class="foot">${esc(me()?.dept||"")} ${esc(me()?.name||"")} · ${new Date().toLocaleString(lang()==="en"?"en-US":"ko-KR")} ${t("출력")}</p></body></html>`;
}
function printDoc(k){
